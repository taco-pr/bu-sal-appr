function [map,chanmaps,maps,chans] = my_simpsal_multiplicative_approach( img , param );
...
if ( isfield(param,'nGaborAngles') && (ismember( 'O' , param.channels ) || ismember( 'S' , param.channels ) ) )
...
levels = param.levels;
imgpyr{1}=img;
sz_ = [size(img,1) size(img,2)];
for i=2:max(levels)
    sz_ = sz_/2;
    imgpyr{i} = my_imresize(img,sz_);
end

% USE only pyramid levels defined in param.levels
count_ = 1;
for i=levels
    imgs{count_} = imgpyr{i};
    count_ = count_ + 1;
end

% define mapSize
mapSize = [size(img,1) size(img,2)]/1;
...
for ci = 1 : numel( param.channels )
  ...
  for cj = 1 : length(chans{ci})    
    ...
    %else wj = 1; end
    wj=1;
    chanmap = chanmap .* imresize(maps{ci}{cj}, mapSize );
    sumW = sumW + wj;
  end
